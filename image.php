		...
		<div class="post">
			<div class="journalism-post-carcas">
				<header>
					...
					<div class="post-data">
						<article>
							<?php _e( 'Posted on', 'journalism' ); ?>&nbsp<?php echo get_the_date(); ?><&nbsp<?php _e( 'in', 'journalism' ); ?>
				...
				<div class="post-text">
					<p><?php _e( 'See in full size: ', 'journalism' ); ?>
						<a href="<?php echo esc_url( wp_get_attachment_url() ); ?>" title="<?php _e( 'full size of ', 'journalism' );
						the_title(); ?>"> <?php $metadata = wp_get_attachment_metadata();
							echo $metadata['width'] . ' &times; ' . $metadata['height'] . ' '; ?></a><?php _e( 'px', 'journalism' ); ?>
					</p>
					...
					the_content();
					wp_link_pages( array(
						'before' => '<div class="page-link"><span>' . __( 'Pages:', 'journalism' ) . '</span>',
						'after'  => '</div>',
					) ); ?>
				</div><!-- div .post-text -->
				<hr />
